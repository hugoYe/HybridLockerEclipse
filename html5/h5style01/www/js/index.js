...
$(function() {
	...
	//判断控制中心是否被打开过
	if (localStorage.Opened_ver_1){
		$("#button_th .redDot").css("display","none");
	}else {
		$("#button_th .redDot").css("display","block");
		window.setTimeout("$('#button_th .redDot').css({'animation':'shake 0.2s linear 2','-webkit-animation':'shake 0.2s linear 2'})",500);
	}
	...
	//为控制中心图标绑定事件
	$("#button_th").bind('click', function(event){
		event.stopPropagation();
		slideUp();
		plugins.EventStatistics.onEvent(ACTION_OPEN_CONTROL_CENTER);
	});
	$("#button_th img").css({
		'animation':'roll 0.5s linear 1','-webkit-animation':'roll 0.5s linear 1'
	});
	window.setTimeout("$('#button_th img').css({'animation':'none'})",500);
})
...
function dragStart(event){
	...
	$(".touch_item img").attr("src","res/arrow_click.png");
	$("#slide").css({
...
function upDragEnd(event){
	$(".touch_item img").attr("src","res/arrow.png");
	$("#slide").css({
...
function slideUp(){
	...
	$("#button_th .redDot").css("display","none");
	if (localStorage.Opened_ver_1){} else {
		localStorage.setItem("Opened_ver_1",true);
	}
	$("#app .app_item img").bind('touchstart',function(event){
		$(this).css({"-webkit-transform":"scale3d(1.1,1.1,1)"});
		event.stopPropagation();
