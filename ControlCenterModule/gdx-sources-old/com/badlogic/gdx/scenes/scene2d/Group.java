...
public class Group extends Actor implements Cullable {
	...
	protected Matrix4 updateTransform () {
		...
		if (originX != 0 || originY != 0)
			localTransform.setToTranslation(originX, originY,0f);
		else
			localTransform.idt();
		if (rotation != 0) localTransform.mul(temp.setToRotation(rotationVector.x,rotationVector.y,rotationVector.z,rotation));
		if (scaleX != 1 || scaleY != 1) localTransform.mul(temp.setToScaling(scaleX, scaleY,1f));
		if (originX != 0 || originY != 0) localTransform.mul(temp.setToTranslation(-originX, -originY,0f));
