...
public class TextureAtlas implements Disposable {
	...
	private ByteBuffer ByteBufferTextureAtlas = null;
	...
	public static class TextureAtlasData {
		...
		public TextureAtlasData(FileHandle packFile, FileHandle imagesDir,
				boolean flip) {
			PriorityQueue<Region> sortedRegions = new PriorityQueue(16,
					indexComparator);
			...
			try {
				...
				while (true) {
					...
					if (line.trim().length() == 0)
						...
						pageImage = new Page(file, min.isMipMap(), format, min,
								max, repeatX, repeatY);
						pages.add(pageImage);
					} else {
	...
	public TextureAtlas(FileHandle packFile, ByteBuffer textureAtlasdataBuffer) {
		this(packFile, packFile.parent(), textureAtlasdataBuffer);
		// ByteBufferTextureAtlas = textureAtlasdataBuffer;
	...
	public TextureAtlas(FileHandle packFile, FileHandle imagesDir,
			ByteBuffer textureAtlasdataBuffer) {
		this(packFile, imagesDir, false, textureAtlasdataBuffer);
	}
	...
	public TextureAtlas(FileHandle packFile, FileHandle imagesDir,
			boolean flip, ByteBuffer textureAtlasdataBuffer) {
		this(new TextureAtlasData(packFile, imagesDir, flip),
				textureAtlasdataBuffer);
	}
	...
	public TextureAtlas(TextureAtlasData data, ByteBuffer textureAtlasdataBuffer) {
		ByteBufferTextureAtlas = textureAtlasdataBuffer;
		load(data);
	}
	...
	private void load(TextureAtlasData data) {
		...
		for (Page page : data.pages) {
			...
			if (page.texture == null) {
				if (page.textureFile.path().endsWith(".pvr")) {
					if (ByteBufferTextureAtlas == null) {
						texture = new BitmapTexture(page.textureFile);
					} else {
						texture = new BitmapTexture(page.textureFile,
								ByteBufferTextureAtlas);
					}
				} else {
		...
		Log.v("xxxxxxxxx", "XXXXXXXXXXXXXXin");
	...
	public AtlasRegion addRegion(String name, TextureRegion textureRegion) {
		return addRegion(name, textureRegion.texture,
	...
	static public class AtlasRegion extends TextureRegion {
		 ...
		 * flipped as if no whitespace has been removed for packing.
		 */
	...
	static public class AtlasSprite extends Sprite {
		...
		public void setBounds(float x, float y, float width, float height) {
			...
			super.setBounds(x + region.offsetX, y + region.offsetY,
					region.packedWidth * widthRatio, region.packedHeight
							* heightRatio);
		}
