...
public class NativeToJsMessageQueue {
	...
	/**
	 * When true, the active listener is not fired upon enqueue. When set to
	 * false, the active listener will be fired if the queue is non-empty.
	 */
	...
	public void setBridgeMode(int value) {
		if (value < -1 || value >= bridgeModes.size()) {
			...
			if (newMode != activeBridgeMode) {
				Log.d(LOG_TAG, "Set native->JS mode to "
						+ (newMode == null ? "null" : newMode.getClass()
								.getSimpleName()));
				synchronized (this) {
	 ...
	 * Combines as many messages as possible, while staying under
	 * MAX_PAYLOAD_SIZE. Returns null if the queue is empty.
	 */
	public String popAndEncode(boolean fromOnlineEvent) {
		synchronized (this) {
			...
			if (!queue.isEmpty()) {
				// Attach a char to indicate that there are more messages
				// pending.
				sb.append('*');
			}
	...
	/**
	 * Same as popAndEncode(), except encodes in a form that can be executed as
	 * JS.
	 */
	...
	public void addPluginResult(PluginResult result, String callbackId) {
		if (callbackId == null) {
			Log.e(LOG_TAG, "Got plugin result with no callbackId",
					new Throwable());
			return;
		}
		// Don't send anything if there is no result and there is no need to
		// clear the callbacks.
		boolean noResult = result.getStatus() == PluginResult.Status.NO_RESULT
				.ordinal();
	...
	private void enqueueMessage(JsMessage message) {
		synchronized (this) {
			if (activeBridgeMode == null) {
				Log.d(LOG_TAG,
						"Dropping Native->JS message due to disabled bridge");
				return;
			}
			queue.add(message);
			if (!paused) {
	...
	public void setPaused(boolean value) {
		if (paused && value) {
			// This should never happen. If a use-case for it comes up, we
			// should
			// change pause to be a counter.
			Log.e(LOG_TAG, "nested call to setPaused detected.",
					new Throwable());
		}
		paused = value;
		if (!value) {
	...
	public static abstract class BridgeMode {
		...
		public void notifyOfFlush(NativeToJsMessageQueue queue,
	...
	public static class LoadUrlBridgeMode extends BridgeMode {
		...
		public LoadUrlBridgeMode(CordovaWebViewEngine engine,
	...
	public static class OnlineEventsBridgeMode extends BridgeMode {
		...
		public void reset() {
			delegate.runOnUiThread(new Runnable() {
				public void run() {
					online = false;
					// If the following call triggers a notifyOfFlush, then
					// ignore it.
		...
		// excess events.
		@Override
		public void notifyOfFlush(final NativeToJsMessageQueue queue,
				boolean fromOnlineEvent) {
	...
	private static class JsMessage {
		...
		static int calculateEncodedLengthHelper(PluginResult pluginResult) {
			...
			case PluginResult.MESSAGE_TYPE_MULTIPART:
				...
				for (int i = 0; i < pluginResult.getMultipartMessagesSize(); i++) {
					int length = calculateEncodedLengthHelper(pluginResult
							.getMultipartMessage(i));
		...
		static void encodeAsMessageHelper(StringBuilder sb,
			...
			case PluginResult.MESSAGE_TYPE_MULTIPART:
				sb.append('M');
				for (int i = 0; i < pluginResult.getMultipartMessagesSize(); i++) {
					PluginResult multipartMessage = pluginResult
							.getMultipartMessage(i);
					sb.append(String
							.valueOf(calculateEncodedLengthHelper(multipartMessage)));
					sb.append(' ');
		...
		void encodeAsMessage(StringBuilder sb) {
			...
			sb.append((noResult || resultOk) ? 'S' : 'F')
					...
					.append(' ').append(jsPayloadOrCallbackId).append(' ');
		...
		void encodeAsJsMessage(StringBuilder sb) {
			if (pluginResult == null) {
				...
				int status = pluginResult.getStatus();
				boolean success = (status == PluginResult.Status.OK.ordinal())
						|| (status == PluginResult.Status.NO_RESULT.ordinal());
				sb.append("cordova.callbackFromNative('")
						.append(jsPayloadOrCallbackId).append("',")
						.append(success).append(",").append(status)
						.append(",[");
				switch (pluginResult.getMessageType()) {
				case PluginResult.MESSAGE_TYPE_BINARYSTRING:
					sb.append("atob('").append(pluginResult.getMessage())
							.append("')");
					break;
				case PluginResult.MESSAGE_TYPE_ARRAYBUFFER:
					sb.append(
							"cordova.require('cordova/base64').toArrayBuffer('")
							.append(pluginResult.getMessage()).append("')");
					break;
				default:
					sb.append(pluginResult.getMessage());
				}
				sb.append("],").append(pluginResult.getKeepCallback())
						.append(");");
			}
		}
	}
