...
public class SystemWebViewEngine implements CordovaWebViewEngine {
	...
	public SystemWebViewEngine(Context context, Context remoteContext,
	...
	public void init(CordovaWebView parentWebView, CordovaInterface cordova,
		...
		nativeToJsMessageQueue
				.addBridgeMode(new NativeToJsMessageQueue.OnlineEventsBridgeMode(
						new NativeToJsMessageQueue.OnlineEventsBridgeMode.OnlineEventsBridgeModeDelegate() {
							...
							public void runOnUiThread(Runnable r) {
								if (SystemWebViewEngine.this.cordova
										.getActivity() != null) {
									SystemWebViewEngine.this.cordova
											.getActivity().runOnUiThread(r);
								} else if (SystemWebViewEngine.this.cordova
										.getCordovaWrap() != null) {
									SystemWebViewEngine.this.cordova
											.getCordovaWrap().runOnUiThread(r);
								}
	...
	private void initWebViewSettings() {
		...
		String databasePath = webView.getContext().getApplicationContext()
		...
		// Debugging!
		ApplicationInfo appInfo = webView.getContext().getApplicationContext()
				.getApplicationInfo();
		...
		// Fix for CB-3360
		String overrideUserAgent = preferences.getString("OverrideUserAgent",
				null);
	...
	private void enableRemoteDebugging() {
		try {
			...
			Log.d(TAG,
					"You have one job! To turn on Remote Web Debugging! YOU HAVE FAILED! ");
			e.printStackTrace();
		}
	}
	...
	private static void exposeJsInterface(WebView webView, CordovaBridge bridge) {
		if ((Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1)) {
			Log.i(TAG,
					"Disabled addJavascriptInterface() bridge since Android version is old.");
			// Bug being that Java Strings do not get converted to JS strings
			// automatically.
			// This isn't hard to work-around on the JS side, but it's easier to
			// just
	...
	public boolean goBack() {
		...
		// parentEngine's history, but not our history url array (JQMobile
		// behavior)
