...
public class PluginManager {
	...
	public PluginManager(CordovaWebView cordovaWebView,
			CordovaInterface cordova, Collection<PluginEntry> pluginEntries) {
		this.ctx = cordova;
	...
	public void addService(PluginEntry entry) {
		...
		if (entry.plugin != null) {
			entry.plugin.privateInitialize(entry.service, ctx, app,
					app.getPreferences());
	...
	/**
	 * Called when the system received an HTTP authentication request. Plugins
	 ...
	 * 
	 * @return Returns True if there is a plugin which will resolve this auth
	...
	public boolean onReceivedHttpAuthRequest(CordovaWebView view,
		...
		for (CordovaPlugin plugin : this.pluginMap.values()) {
			if (plugin != null
					&& plugin.onReceivedHttpAuthRequest(app, handler, host,
							realm)) {
				return true;
			}
		}
	...
	public boolean onReceivedClientCertRequest(CordovaWebView view,
		...
		for (CordovaPlugin plugin : this.pluginMap.values()) {
			if (plugin != null
					&& plugin.onReceivedClientCertRequest(app, request)) {
				return true;
			}
		}
		return false;
	 ...
	 * @return Returns true to allow the resource to load, false to block the
	 *         resource.
	 */
	 ...
	 * @param url
	 *            The URL that is being requested.
	 * @return Returns true to allow the navigation, false to block the
	 *         navigation.
	 */
	...
	/**
	 * Called when the webview is going not going to navigate, but may launch an
	 * Intent for an URL.
	 * 
	 * This delegates to the installed plugins, and returns true/false for the
	 * first plugin to provide a non-null result. If no plugins respond, then
	 ...
	 * @param url
	 *            The URL that is being requested.
	 * @return Returns true to allow the URL to launch an intent, false to block
	 *         the intent.
	 */
