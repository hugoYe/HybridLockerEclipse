...
public class Whitelist {
	private static class URLPattern {
		...
		public URLPattern(String scheme, String host, String port, String path)
			...
			try {
				...
				if ("*".equals(host)) {
					...
					this.host = Pattern.compile("([a-z0-9.-]*\\.)?"
							+ regexFromPattern(host.substring(2), false),
							Pattern.CASE_INSENSITIVE);
				} else {
		...
		public boolean matches(Uri uri) {
			try {
				return ((scheme == null || scheme.matcher(uri.getScheme())
						.matches())
						&& (host == null || host.matcher(uri.getHost())
								.matches())
						&& (port == null || port.equals(uri.getPort())) && (path == null || path
	 ...
	 * 
	 * <url-pattern> := <scheme>://<host><path> <scheme> := '*' | 'http' |
	 * 'https' | 'file' | 'ftp' | 'chrome-extension' <host> := '*' | '*.' <any
	 * char except '/' and '*'>+ <path> := '/' <any chars>
	 * 
	 * We extend this to explicitly allow a port attached to the host, and we
	 * allow the scheme to be omitted for backwards compatibility. (Also host is
	 * not required to begin with a "*" or "*.".)
	 */
