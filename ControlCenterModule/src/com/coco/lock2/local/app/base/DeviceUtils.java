...
public class DeviceUtils {
		...
		if (activity != null) {
			...
			if (SDK_INT == 13) {
				try {
					...
					e.printStackTrace();
					Log.e(TAG,
							"getRealScreenPixelsHeight failed --- SDK_INT == 13 !",
							e);
				}
				...
				try {
					...
					e.printStackTrace();
					Log.e(TAG,
							"getRealScreenPixelsHeight failed --- getRawHeight !",
							e);
				}
			} else if (SDK_INT >= 17) {
	...
	public static boolean isDoovL1() {
		if (DEVICE_MANUFACTURER.equalsIgnoreCase("doov")
				&& DEVICE_MODEL.contains("doov l1")) {
			return true;
		}
		return false;
	}
	...
	public static boolean hasMeiZuSmartBar() {
		...
		try {
			...
			Log.w(TAG, "hasMeiZuSmartBar--noSuchMethod(hasSmartBar)", e);
		}
