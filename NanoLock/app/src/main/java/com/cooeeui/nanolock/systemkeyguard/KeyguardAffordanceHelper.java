...
//                                                                 android.R.interpolator.fast_out_linear_in);
    ...
    private View getIconAtPosition(float x, float y) {
        if (mCallback.getLeftPreview() != null
            && mCallback.getLeftPreview().getVisibility() == View.VISIBLE
            || mCallback.getRightPreview() != null
               && mCallback.getRightPreview().getVisibility() == View.VISIBLE) {
            return null;
        }
    ...
    public boolean shouldReset(float x, float y) {

        View preview = null;
        if (mCallback.getLeftPreview() != null
            && mCallback.getLeftPreview().getVisibility() == View.VISIBLE) {
            preview = mCallback.getLeftPreview();
        } else if (mCallback.getRightPreview() != null
                   && mCallback.getRightPreview().getVisibility() == View.VISIBLE) {
            preview = mCallback.getRightPreview();
        }

        int[] point = new int[2];
        if (preview != null) {
            preview.getLocationOnScreen(point);
            if (y < point[1] || x < point[0] || x > point[0] + preview.getWidth()) {
                return true;
            }
        }
