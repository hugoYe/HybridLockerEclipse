...
public class CustomerHttpClientNew {
	private static final String CHARSET = HTTP.UTF_8;
	private static HttpClient customerHttpClient;
	private static Context context;

	public CustomerHttpClientNew(Context context) {
	...
	private static synchronized HttpClient getHttpClient() {
		if (null == customerHttpClient) {
			...
			schReg.register(new Scheme("https",
					(SocketFactory) SSLSocketFactory.getSocketFactory(), 443));
	...
	public static String[] post(String url, String content, Context c) {

		if (isNetworkAvailable(c) == false) {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				String strResult = (resEntity == null) ? null : EntityUtils
						.toString(resEntity, CHARSET);
				Log.v("http", "customerHttpClient post error = "
