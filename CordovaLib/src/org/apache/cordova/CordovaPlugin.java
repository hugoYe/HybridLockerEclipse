...
public class CordovaPlugin {
	...
	/**
	 * Call this after constructing to initialize the plugin. Final because we
	 * want to be able to change args without breaking plugins.
	 */
	public final void privateInitialize(String serviceName,
	...
	/**
	 * Returns the plugin's service name (what you'd use when calling
	 * pluginManger.getPlugin())
	 */
	 ...
	 * to block the request: if any plugin returns false, Cordova will block the
	 * request. If all plugins return null, the default policy will be enforced.
	 * If at least one plugin returns true, and no plugins return false, then
	...
	/**
	 * Hook for allowing page to call exec(). By default, this returns the
	 * result of shouldAllowNavigation(). It's generally unsafe to allow
	 * untrusted content to be loaded into a CordovaWebView, even within an
	 * iframe, so it's best not to touch this.
	 */
	...
	/**
	 * Called to handle CordovaResourceApi.openForRead() calls for a
	 * cdvplugin://pluginId/ URL. Should never return null. Added in
	...
	/**
	 * Refer to remapUri() Added in cordova-android@4.0.0
	 */
	protected Uri toPluginUri(Uri origUri) {
		return new Uri.Builder().scheme(CordovaResourceApi.PLUGIN_URI_SCHEME)
				.authority(serviceName)
				.appendQueryParameter("origUri", origUri.toString()).build();
	}
	 ...
	 * can use the supplied HttpAuthHandler to process this auth challenge.
	 * 
	 * @param view
	 ...
	 * 
	 * @return Returns True if plugin will resolve this auth challenge,
	 *         otherwise False
	 * 
	 */
	...
	/**
	 * Called when he system received an SSL client certificate request. Plugin
	 * can use the supplied ClientCertRequest to process this certificate
	 * challenge.
	 * 
	 * @param view
	 *            The WebView that is initiating the callback
