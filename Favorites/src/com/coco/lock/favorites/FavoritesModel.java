...
public class FavoritesModel extends BroadcastReceiver {
	...
	public static int DEFAULT_FAVORITE_NUM = 8;
	...
	public static void loadFavoritesFromDb() {
		...
		final Cursor c = mFavoritesDbOperation.query(FavoritesDatabaseOperation.TABLE_FAVORITE, null, where, null,
				null);
		try {
			final int idIndex = c
	...
	public static void updateItemInDatabase(Context context,
		...
		values.put(FavoritesDatabaseOperation.Favorites.LAUNCH_TIMES,
			...
			Runnable r = new Runnable() {
				...
				public void run() {
					mFavoritesDbOperation.update(FavoritesDatabaseOperation.TABLE_FAVORITE, values, FavoritesDatabaseOperation.Favorites._ID + " = ?", new String[]{Long.toString(appinfo.id)});
				}
			};
			mHandler.post(r);
	}
	...
	public static void deleteItemFromDatabase(Context context,
		...
		if (item == null || getFavoritesProvider() == null) {
			...
			Runnable r = new Runnable() {
				...
				public void run() {
					mFavoritesDbOperation.delete(FavoritesDatabaseOperation.TABLE_FAVORITE, FavoritesDatabaseOperation.Favorites._ID + " = ?", new String[]{Long.toString(item.id)});
				}
			};
			mHandler.post(r);
	}
