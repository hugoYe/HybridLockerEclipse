...
public class DownloadService extends Service {
	...
	private final String DOWNLOAD_PATH = Environment
			.getExternalStorageDirectory() + "/CooeeLocker/download/";
		...
		public void handleMessage(Message msg) {
			...
			case DOWNLOAD_COMPLETE: {
				...
				Notification finishNotification = new Notification(
						R.drawable.logo, "", System.currentTimeMillis());
				finishNotification.setLatestEventInfo(DownloadService.this,
						downloadRunnable.mDownloadFileName, getResources()
								.getString(R.string.server_download_finish),
						downCompletedIntent);
			...
			case DOWNLOAD_FAIL: {
				...
				Notification failNotification = new Notification(
						R.drawable.logo, "", System.currentTimeMillis());
				failNotification.setLatestEventInfo(DownloadService.this,
						downloadRunnable.mDownloadFileName, getResources()
								.getString(R.string.server_download_fail),
	...
	private class DownloadRunnable implements Runnable {
		...
		public DownloadRunnable(String downloadUrl, String downloadFileName) {
			...
			mUpdatePendingIntent = PendingIntent.getActivity(
					DownloadService.this, 0, mUpdateIntent, 0);

			String tickerText = getResources().getString(
					R.string.server_begin_download);
			mUpdateNotification = new Notification(R.drawable.logo, tickerText,
					System.currentTimeMillis());
			mUpdateNotification.setLatestEventInfo(DownloadService.this,
				...
				if (!mDownloadFile.exists()) {
					...
					try {
						if (!mDownloadFile.delete()) {
							mDownloadFile = new File(DOWNLOAD_PATH,
									mDownloadFileName + mNotifyId + ".apk");
						}
						mDownloadFile.createNewFile();
		...
		private long downloadFile(String downloadUrl, File saveFile)
			...
			try {
				...
				if (LoadLockBoxActivity.isCWWAPConnect(getApplicationContext())
						&& LoadLockBoxActivity
								.getNetWorkType(getApplicationContext()) != NETTYPE_UNICOM) {

					String host = LoadLockBoxActivity
							.getProxyHost(getApplicationContext());
					int port = LoadLockBoxActivity
				...
				String loading = getResources().getString(
						R.string.server_downloading);
				int readsize = 0;
				int percentCount = 0;
				int curPercent = 0;
