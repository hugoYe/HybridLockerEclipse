...
public class JSClass {
	...
	public JSClass(WebView webView, String moudleName, int dlManagerID,
			int entryID, Handler MainActivityHandler, MyR R,
			MeApkDLShowType curShowType) {
		this.context = webView.getContext();
		this.webView = webView;
		this.moudleName = moudleName;
		this.mDLManagerID = dlManagerID;
		this.mEntryId = entryID;
		this.MainActivityHandler = MainActivityHandler;
		this.R = R;
		this.curShowType = curShowType;
		MeApkDlMgr = MeApkDlMgrBuilder.Build(context.getApplicationContext(),
				moudleName, mDLManagerID);
	 ...
	 * 1: install 0: remove
	 */
	public void appInstallInfoChange(Context context, String packageName,
			int installOrRemove) {
		try {
			if (installOrRemove == 0) {
				infoMap.remove(packageName);
			} else {
				infoMap.put(packageName, context.getPackageManager()
	...
	public int checkAppIsInstall(String packageName) {
	...
	@JavascriptInterface
	public boolean invokeJSMethod(String methodName, Object... params) {
		try {
	...
	public int getVersionCode(String pageName, Context context) {
		try {
	...
	// 获取APK是否安装
	@JavascriptInterface
	public int AppQueryState(String pkgName) {
		dl_info curDLInfo = MeApkDlMgr.GetSdkApkDlMgr().ResGetInfo(
				CoolDLResType.RES_TYPE_APK, pkgName);
		int res = MeApkDlMgr.GetSdkApkDlMgr().ResQueryState(
				CoolDLResType.RES_TYPE_APK, pkgName);
		// 如果已安装完成
		if (res == 3) {
			// 如果有下载记录
			if (null != curDLInfo) {
				...
				// 有下载记录，下载完成，但未安装
					...
					int CurVersionCode = (Integer) curDLInfo
	...
	public void subWebviewBack() {
		MainActivityHandler
				.sendEmptyMessage(MainActivity.subWebViewBackSoftKey);
	}

	@JavascriptInterface
	public void OpenSubWebview(String subUrl) {
		// TODO Auto-generated method stub
		MELOG.v("ME_RTFSC", "subUrl:" + subUrl);
		// loadUrl(subUrl );
		Message msg = new Message();
		msg.what = MainActivity.openSubWebView;
		msg.obj = subUrl;
	...
	public void setDialog() {
		WebviewLoadingDlg.ShowDlg(context);
		// builder = new ProgressDialog( context );
		// //builder.setIcon( R.drawable.ic_launcher );
		// builder.setMessage( context.getString( R.string.cool_ml_loading_1 )
		// );
	...
	MeApkDownloadCallBack WebMainApkDownloadCallBack = new MeApkDownloadCallBack(
			curShowType) {
		...
		void onFail(String PackageName, dl_info info) {
			...
			new CoolLog(context).v("ME_RTFSC", "webPage  onFail");
		...
		void onDoing(String PackageName, dl_info info) {
			...
			invokeJSMethod("DownloadProgress", PackageName, info.getCurBytes(),
					info.getTotalBytes());
		}
	...
	@JavascriptInterface
	public int DownloadApk(final String pkgName, final String json) {
		MELOG.v("ME_RTFSC", "=============== DownloadApk =================== ");
		// webview 中的JSINterface 回调函数必须和 webview在同一进程中
		...
		webView.post(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				MeApkDlMgr.GetSdkIconMgr().IconDownload(pkgName,
						new CoolDLCallback() {

							@Override
							public void onSuccess(CoolDLResType arg0,
									String arg1, dl_info arg2) {
								// TODO Auto-generated method stub
								Intent intent = new Intent(
										"com.cooee.microEntrce.download");
								intent.putExtra("pkgName", pkgName);
								context.sendBroadcast(intent);
							}

							@Override
							public void onFail(CoolDLResType arg0, String arg1,
									dl_info arg2) {
								// TODO Auto-generated method stub
							}

							@Override
							public void onDoing(CoolDLResType arg0,
									String arg1, dl_info arg2) {
								// TODO Auto-generated method stub
							}
						});
				if (pkgName.startsWith("http://")
					...
					dl_cb dl = new dl_cb() {
						...
						@Override
						public void onSuccess(dl_info info) throws Exception {
							// TODO Auto-generated method stub
							...
							// AppInstall( pkgName );
							MeGeneralMethod.InstallApk(context,
									info.getFilePath());
						}

						@Override
						public void onStart(dl_info info) throws Exception {
							// TODO Auto-generated method stub
						}

						@Override
						public void onFail(dl_result result, dl_info info)
								throws Exception {
							// TODO Auto-generated method stub
						...
						@Override
						public void onDoing(dl_info info) throws Exception {
							// TODO Auto-generated method stub
							invokeJSMethod("DownloadProgress", pkgName,
									info.getCurBytes(), info.getTotalBytes());
						}
					};
					MeApkDlMgr.GetSdkApkDlMgr().UrlDownload(pkgName, dl);
					return;
				}
				if (null == MeApkDlMgr.GetSdkApkDlMgr().ResGetInfo(
						CoolDLResType.RES_TYPE_APK, pkgName)) {
					try {
						...
						MeApkDlMgr.StartDownload(curShowType, pkgName, appName,
								size, versionName, mEntryId,
								WebMainApkDownloadCallBack);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						MELOG.e("ME_RTFSC",
								"DownloadApk:onDoing Error:" + e.toString());
						e.printStackTrace();
					}
				} else if (0 == MeApkDlMgr.GetSdkApkDlMgr()
						.ResGetInfo(CoolDLResType.RES_TYPE_APK, pkgName)
						.getDownloadState()) {
					MeApkDlMgr.ReStartDownload(curShowType, pkgName,
							WebMainApkDownloadCallBack);
				} else {
					MeApkDlMgr.AddCallback(curShowType, pkgName,
							WebMainApkDownloadCallBack);
				}
			}
		});
	...
	// }
	@JavascriptInterface
	public void DownloadStop(final String pkgName) {
		Thread th = new Thread(new Runnable() {
	...
	// 获取已经安装的APK信息
	@JavascriptInterface
	public String AppQueryInfo(String pkgName) {
		...
		try {
			if (null != info) {
				...
				json.put("installLocation",
						info.applicationInfo.sourceDir.toString());
				json.put("cardBytes",
						info.applicationInfo.publicSourceDir.toString());
		...
		try {
			for (int i = 0; i <= apps.size() - 1; i++) {
				...
				if ((apps.get(i).applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) <= 0) {
					...
					json.put("installLocation",
							apps.get(i).applicationInfo.sourceDir.toString());
					json.put("cardBytes",
							apps.get(i).applicationInfo.publicSourceDir
									.toString());
	...
	public static Boolean IsNetworkAvailableLocal(Context context) {
	...
	// }
	@JavascriptInterface
	public void AppStart(String packageName) {
	...
	@JavascriptInterface
	public int AppInstall(String PackageName) {
		MELOG.v("ME_RTFSC", " ====== AppInstall   webpage===== ");
		dl_info info = MeApkDlMgr.GetSdkApkDlMgr().ResGetInfo(
				CoolDLResType.RES_TYPE_APK, PackageName, true);
		if (info != null) {
	...
	public void SetBackgroundWithWallpaper() {
		Message msg = new Message();
		msg.what = MainActivity.setBackgroundWithWallpaper;
		msg.obj = webView;
		MainActivityHandler.sendMessage(msg);
	}
	...
	public void DlMgrOpen() {
		...
		intent.putExtra("entryId", mDLManagerID);
		context.startActivity(intent);
	}
	...
	@JavascriptInterface
	// 判断googleplay是否已安装
	public int googlePalyIsInstall() {
		return Utility.googlePalyIsInstall(context);
	}
	...
	@JavascriptInterface
	public void LoadUrlByBrowserByGP(String url) {
		try {
	...
	@JavascriptInterface
	public boolean startAppStoreDownload(String pkgName) {
		try {
			context.startActivity(new Intent(Intent.ACTION_VIEW).setData(Uri
					.parse("market://details?id=" + pkgName)));
			MELOG.v("ME_RTFSC", " ====== appstore start===== ");
			return true;
		} catch (Exception e2) {
			MELOG.v("ME_RTFSC",
					" ====== start appstore Activity error:" + e2.getMessage());
	...
	/*
	 * private boolean isGoogleLink( String url ) { return ( url.indexOf(
	 * "https://play.google" ) >= 0 ) || ( url.indexOf( "http://play.google" )
	 * >= 0 ); }
	 */
	private List<ResolveInfo> getAllInstallBrowser(PackageManager packageMgr) {
		Intent intent = (new Intent(Intent.ACTION_VIEW, Uri.parse("http://")));
		intent.addCategory(Intent.CATEGORY_BROWSABLE);
		List<ResolveInfo> allMatches = packageMgr.queryIntentActivities(intent,
				PackageManager.MATCH_DEFAULT_ONLY);
		return allMatches;
	}

	private void choiceBrowserToVisitUrl(Context context, String url,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			} else if (temp.equals("com.baidu.browser.apps"))// com.baidu.browser.cloud.launcher.RECEIVE
			{
				baiduPath = temp;
				existBaidu = true;
			} else if (temp.equals("com.opera.browser.classic")
					|| temp.equals("com.oupeng.mini.android"))// com.opera.mini.android
			{
				// 存在Opera
				operaPath = temp;
				existOpera = true;
			} else if (temp.equals("org.mozilla.firefox_beta")
					|| temp.equals("org.mozilla.firefox")) {
				fireboxPath = temp;
				existFirebox = true;
	...
	private ResolveInfo getUserDefinitionBrowers(PackageManager packageMgr) {
		Intent intent = (new Intent(Intent.ACTION_VIEW, Uri.parse("http://")));
		intent.addCategory(Intent.CATEGORY_BROWSABLE);
		ResolveInfo bestMatch = packageMgr.resolveActivity(intent,
				PackageManager.MATCH_DEFAULT_ONLY);
	...
	private void doDefaultBrowser(Context context, String visitUrl) {
		try {
	...
	private void gotoUrl(Context context, String packageName, String url,
			PackageManager packageMgr) {
		try {
	...
	@JavascriptInterface
	public String h14() {
		return MceInfo.instance(context).getH14();
	}
