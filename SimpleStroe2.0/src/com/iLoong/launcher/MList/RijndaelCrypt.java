...
public class RijndaelCrypt {
	 ...
	 * @password Public key
	 */
	public RijndaelCrypt(String password, byte[] IV) {
		try {
			// Encode digest
	 ...
	 */
	public byte[] encryptToByteArray(byte[] text) {
		byte[] encryptedData;
		try {
			...
			Log.e(TAG, "Invalid or inappropriate algorithm parameters for "
					+ ALGORITHM, e);
			return null;
	...
	public static void test() {
		...
		RijndaelCrypt aes = new RijndaelCrypt(RijndaelCrypt.PWD,
				RijndaelCrypt.IV);
		try {
			encryStr = aes.encryptToByteArray(testStr.toString().getBytes(
					HTTP.UTF_8));
