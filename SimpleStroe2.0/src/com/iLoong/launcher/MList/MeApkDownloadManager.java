...
public class MeApkDownloadManager {
	...
	public MeApkDownloadManager(Context context, String moudleName, int entryId) {
	...
	public synchronized dl_info GetInfoByPkgName(String PkgName) {
		// TODO Auto-generated method stub
		return apkDlMgr.ResGetInfo(CoolDLResType.RES_TYPE_APK, PkgName);
	}

	public void StartDownload(MeApkDLShowType ShowType, String PkgName,
			String appName, String size, String versionName, int entryID,
			MeApkDownloadCallBack CallBack) {
		// 下载任务的开始只能够从Web的主页面开始
		MeApkDownloadTask CurTask = new MeApkDownloadTask(ShowType, PkgName,
				CallBack, appName, size, versionName, entryID);
		MeApkDownloadTaskMap.put(PkgName, CurTask);
		MeApkDlNotifyManager.getInstance(context.getApplicationContext())
				.onMeApkDlStart(entryId, moudleName, PkgName);
		...
		// MeGeneralMethod.startMeDLProtectionService(
		// context.getApplicationContext() );
		// CurTask.CurCallback.onstart( ShowType , PkgName );
	}

	public void ReStartDownload(MeApkDLShowType ShowType, String PkgName,
			MeApkDownloadCallBack CallBack) {
		MeApkDownloadTask MECurDLTask = MeApkDownloadTaskMap.get(PkgName);
		if (null != MECurDLTask && null != MECurDLTask.CurCallback
				&& null != MECurDLTask.CurSDKDLTask) {
	...
	public void AddCallback(MeApkDLShowType ShowType, String PkgName,
			MeApkDownloadCallBack CallBack) {
		MeApkDownloadTask CurTask = MeApkDownloadTaskMap.get(PkgName);
		CurTask.SetTaskCallback(ShowType, CallBack);
	}

	public void StopDownload(MeApkDLShowType ShowType, String PkgName) {
		MeApkDownloadTask CurTask = MeApkDownloadTaskMap.get(PkgName);
		CurTask.MeApkStopDownload(ShowType, PkgName);
	}

	public void DelDownload(MeApkDLShowType ShowType, String PkgName) {
		...
		MeApkDlNotifyManager.getInstance(context.getApplicationContext())
				.onMeApkDlDel(entryId, moudleName, PkgName);
		// 再删除
		apkDlMgr.ResDownloadStop(CoolDLResType.RES_TYPE_APK, PkgName, true);
		for (MeApkDLShowType type : MeAddActiveCallBackMap.keySet()) {
			if (!type.equals(ShowType)) {
				MELOG.v("ME_RTFSC", "MeApkDownloadManager DelDownload type = "
						+ type);
	...
	public void ApkInstalled(String pkgName) {
		// TODO Auto-generated method stub
		...
		if (null != info && true == info.IsDownloadSuccess()) {
			for (MeApkDLShowType type : MeAddActiveCallBackMap.keySet()) {
				MELOG.v("ME_RTFSC", "MeApkDownloadManager ApkInstalled type = "
						+ type);
				MeAddActiveCallBackMap.get(type).NotifyInstallSucessAction(
						pkgName);
			}
			MeApkDlNotifyManager.getInstance(context.getApplicationContext())
					.onMeApkInstalled(entryId, moudleName, pkgName);
		} else if (null == info) {
			for (MeApkDLShowType type : MeAddActiveCallBackMap.keySet()) {
				MELOG.v("ME_RTFSC", "MeApkDownloadManager ApkInstalled type = "
						+ type);
	...
	public void ApkUninstall(String pkgName) {
		// TODO Auto-generated method stub
		for (MeApkDLShowType type : MeAddActiveCallBackMap.keySet()) {
			MELOG.v("ME_RTFSC", "MeApkDownloadManager DelDownload type = "
					+ type);
	...
	public synchronized int GetUninstallApkCount() {
		...
		for (dl_info info : ApkTaskList) {
			...
			if (!info.IsDownloadSuccess()) {
				...
				int versionCode = getVersionCode((String) info.getValue("r4"),
						context);
				// MELOG.v( "ME_RTFSC" , "----CurVersionCode:" +CurVersionCode +
	...
	class MeApkDownloadTask {
		...
		public MeApkDownloadTask(MeApkDLShowType ShowType, String PackageName,
				MeApkDownloadCallBack CallBack, String appName, String size,
				String versionName, int entryID) {
			// TODO Auto-generated constructor stub
			...
			CurSDKDLTask = apkDlMgr.ResDownloadNewTask(
					CoolDLResType.RES_TYPE_APK, PackageName, CurCallback,
					MicroEntry.h12, MicroEntry.h13 + entryID);
			...
			if (!Environment.getExternalStorageState().equals(
					...
					|| 2 == apkDlMgr.ResDownloadStart(CurSDKDLTask)) {
				CurCallback.onFail(CoolDLResType.RES_TYPE_APK, PackageName,
						null);
			}
		}
		...
		public void MeApkReStartDownload(MeApkDLShowType ShowType,
				String PackageName, MeApkDownloadCallBack CallBack) {
			MELOG.v("ME_RTFSC", "MeApkDownloadTask:MeApkReStartDownload");
			// TODO Auto-generated method stub
			CurCallback.AddMECallBack(ShowType, CallBack);
			if (!Environment.getExternalStorageState().equals(
					android.os.Environment.MEDIA_MOUNTED)
					|| 2 == apkDlMgr.ResDownloadStart(
							CoolDLResType.RES_TYPE_APK, PackageName,
							CurCallback)) {
				CurCallback.onFail(CoolDLResType.RES_TYPE_APK, PackageName,
						null);
			} else {
		...
		public void SetTaskCallback(MeApkDLShowType ShowType,
				MeApkDownloadCallBack CallBack) {
			MELOG.v("ME_RTFSC", "MeApkDownloadTask:SetTaskCallback");
			CurCallback.AddMECallBack(ShowType, CallBack);
		}

		public void MeApkStopDownload(MeApkDLShowType ShowType,
				String PackageName) {
