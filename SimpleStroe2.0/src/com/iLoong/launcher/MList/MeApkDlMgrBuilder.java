...
public class MeApkDlMgrBuilder {
	...
	public static MeApkDownloadManager Build(Context context,
			String apkmoudleName, int entryId) {
		...
		MeApkDownloadManager CurManager = MeApkDownloadManagerMap
				.get(CurManagerKey);
		if (null == CurManager) {
			CurManager = new MeApkDownloadManager(context, apkmoudleName,
					entryId);
	...
	public static MeApkDownloadManager GetMeApkDownloadManager(int entryId) {
		String CurManagerKey = "MeDlMgr" + entryId;
		if (null == MeApkDownloadManagerMap) {
			return null;
		}
