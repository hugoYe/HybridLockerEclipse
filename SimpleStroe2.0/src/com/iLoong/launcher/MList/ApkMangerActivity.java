...
public class ApkMangerActivity extends Activity {
	...
	int entryId;
	private boolean isStartByNotiy = false;
	String moudleName;
			...
			try {
				...
				case 1:// 下载进度更新
				{
					...
					if (null == CurItem.getIconImgSrc()
							&& null != MeapkDlMgr.GetSdkIconMgr().IconGetInfo(
									CurItem.getPackageName())) {
						String ImgPath = MeapkDlMgr.GetSdkIconMgr()
								.IconGetInfo(CurItem.getPackageName())
								.getFilePath();
						if (null != ImgPath && ImgPath.length() > 1) {
							CurItem.setIconImgSrc(Drawable
				...
				case 5:// 删除下载项
				{
					...
					// CurDownloadItem.getPackageName() );
					// }
					// catch( Exception e )
					// {
					// // TODO: handle exception
					// MELOG.e( "ME_RTFSC" , "DownloadUpdateHander 5 Exception:"
					// + e.toString() );
					// }
				...
				{
					MELOG.v("ME_RTFSC",
							" ======  cool_ml_download_failed  ApkManager===== ");
					...
					Toast.makeText(getApplicationContext(), InfoID,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			moudleName = intent.getStringExtra("moudleName");
			entryId = intent.getIntExtra("entryId", -1);
			isStartByNotiy = intent.getBooleanExtra("isStartByNotify", false);
			MELOG.v("ME_RTFSC", "1111 entryId:" + entryId);
			if (entryId == -1) {
				finish();
				return;
			}
			MeapkDlMgr = MeApkDlMgrBuilder.Build(getApplicationContext(),
					moudleName, entryId);
			instance = this;
			CurEntryID = entryId;
			MELOG.v("ME_RTFSCX", "ApkMangerActivity onCreate ===  instance:"
					+ instance + ",CurEntryID:" + CurEntryID);
		} catch (Exception e) {
			finish();
			return;
		}
		...
		flDownloadNUllDate = (FrameLayout) viewDownload
				.findViewById(R.id.cool_ml_flDownloadNULLData);
		flDownlaodListDate = (FrameLayout) viewDownload
				.findViewById(R.id.cool_ml_flDownloadList);
		lvDownloadListView = (ListView) viewDownload
				.findViewById(R.id.cool_ml_lvDownload);
		flInstallNUllDate = (FrameLayout) viewInstall
				.findViewById(R.id.cool_ml_flInsatllNULLData);
		flInstallListDate = (FrameLayout) viewInstall
				.findViewById(R.id.cool_ml_flInsatllList);
		lvInstallListView = (ListView) viewInstall
				.findViewById(R.id.cool_ml_lvInstall);
		// MeCoolDLCallback.MeAddActiveCallBackMap.put(
		// MeApkDLShowType.DownloadManagerActivity , new MeActiveCallback() {
		//
		// @Override
		...
		// MeapkDlMgr.AddCallback( MeApkDLShowType.DownloadManagerActivity ,
		// pkgname , curItem.getDownloadCallback() );
		...
		MeApkDownloadManager.MeAddActiveCallBackMap.put(
				...
				new MeActiveCallback() {

					@Override
					public void NotifyUninstallApkAction(String pkgname) {
						// TODO Auto-generated method stub
						for (int i = 0; i < InstallApkList.size(); i++) {
							InstallApkItemEx CurInstallApkItem = InstallApkList
									.get(i);
							if (pkgname.equals(CurInstallApkItem
									.getPackageName())) {
					...
					public void NoifySatrtAction(String pkgname) {
						// TODO Auto-generated method stub
						for (int i = 0; i < DownloadApkList.size(); i++) {
							if (DownloadApkList.get(i).getPackageName()
									.equals(pkgname)) {
								DownloadApkItem curItem = DownloadApkList
										.get(i);
								MeapkDlMgr
										.AddCallback(
												MeApkDLShowType.DownloadManagerActivity,
												pkgname,
												curItem.getDownloadCallback());
								Message DownloadControlButClickMsg = new Message();
								// 4 -- 开始或者暂停下载
								DownloadControlButClickMsg.what = 4;
								DownloadControlButClickMsg.obj = curItem;
								DownloadUpdateHander
										.sendMessage(DownloadControlButClickMsg);
							}
						}
					}
				});

		DownloadApkNeedDownload();
	}
	...
	public String getVersion(String pageName, Context context) {
		try {
	...
	private void InitInstallList() {
		...
		try {
			...
			ApkTaskList = MeapkDlMgr.GetSdkApkDlMgr().ResGetTaskList(
					CoolDLResType.RES_TYPE_APK);
			MELOG.v("ME_RTFSC", "ApkTaskList.size()" + ApkTaskList.size());
			// IconTaskList =
			// apkDlMgr.ResGetTaskList(CoolDLResType.RES_TYPE_PIC);
			InstallApkList.clear();
			for (dl_info info : ApkTaskList) {
				if (info.IsDownloadSuccess()) {
					if (null == (String) info.getValue("r4")) {
						MELOG.v("ME_RTFSC",
								" null == (String)info.getValue( 'r4')");
						continue;
					}
					...
					InstallApkItem.setAppVersion((String) info
							.getValue("versionName"));
					if (null != MeapkDlMgr.GetSdkIconMgr().IconGetInfo(
							InstallApkItem.getPackageName())) {
						String ImgPath = MeapkDlMgr.GetSdkIconMgr()
								.IconGetInfo(InstallApkItem.getPackageName())
								.getFilePath();
						if (null != ImgPath && ImgPath.length() > 1) {
							InstallApkItem.setIconImgSrc(Drawable
									.createFromPath(ImgPath));
						}
					}
					// int InstallState; /* 0:初始化状态 1:安装 2:启动 */
					if (null != InstallApkItem.getAppVersion()
							&& true == InstallApkItem.getAppVersion().equals(
									getVersion(InstallApkItem.getPackageName(),
											ApkMangerActivity.this))) {
		...
		try {
			...
			for (dl_info info : ApkTaskList) {
				...
				if (!info.IsDownloadSuccess() || 0 != info.getDownloadState()) {
					if (null == (String) info.getValue("r4")
							&& null == (String) info.getValue("p2")) {
						MELOG.v("ME_RTFSC",
								" null == (String)info.getValue( 'r4' ) && null == (String)info.getValue( 'p2' )");
						continue;
					}
					DownloadApkItem downloadApkItem = new DownloadApkItem();
					if (null != (String) info.getValue("r4")) {
						downloadApkItem.setPackageName((String) info
								.getValue("r4"));
					} else {
						downloadApkItem.setPackageName((String) info
								.getValue("p2"));
					...
					if (null != MeapkDlMgr.GetSdkIconMgr().IconGetInfo(
							downloadApkItem.getPackageName())) {
						String ImgPath = MeapkDlMgr.GetSdkIconMgr()
								.IconGetInfo(downloadApkItem.getPackageName())
								.getFilePath();
						if (null != ImgPath && ImgPath.length() > 1) {
							downloadApkItem.setIconImgSrc(Drawable
									.createFromPath(ImgPath));
						}
					}
					downloadApkItem.setDownloadCallback(new DownLoadCallBack(
							MeApkDLShowType.DownloadManagerActivity,
							downloadApkItem));
					// int DownLoadState; //0:未在下载(none) 1:排队等待(wait)
					// 2:正在下载(doing)
					...
					if (0 != info.getDownloadState()) {
						// apkDlMgr.ResSetTaskCallback( info ,
						// downloadApkItem.getPackageName() ,
						// downloadApkItem.getDownloadCallback() );
						MeapkDlMgr.AddCallback(
								MeApkDLShowType.DownloadManagerActivity,
								downloadApkItem.getPackageName(),
								downloadApkItem.getDownloadCallback());
					}
			...
			MELOG.v("ME_RTFSC",
					"DownloadApkList.size()" + DownloadApkList.size());
		} catch (Exception e) {
			// TODO: handle exception
	...
	class DownloadOnItemLongClickListener implements OnItemLongClickListener {
		...
		public boolean onItemLongClick(AdapterView<?> arg0, View arg1,
			...
			// TODO Auto-generated method stub
			MELOG.v("ME_RTFSC",
					"==== lvDownloadListView.setOnItemLongClickListener===");
			LongClickListenerDownloadItem = null;
			LongClickListenerDialog = null;
			LongClickListenerDownloadItem = DownloadApkList.get(arg2);
			LongClickListenerDialog = new AlertDialog.Builder(
					ApkMangerActivity.this).create();
			if (null != LongClickListenerDownloadItem
					&& null != LongClickListenerDialog) {
				LayoutInflater inflaterDl = LayoutInflater
						.from(ApkMangerActivity.this);
				LinearLayout layout = (LinearLayout) inflaterDl.inflate(
						R.layout.cool_ml_onlongclick_listview, null);
				TextView t1 = (TextView) layout
						.findViewById(R.id.cool_ml_appListTextView1);
				TextView t2 = (TextView) layout
						.findViewById(R.id.cool_ml_appListTextView2);
				// DownLoadState 0:未在下载(none) 1:排队等待(wait) 2:正在下载(doing)
				if (0 != LongClickListenerDownloadItem.getDownLoadState()) {
					...
					t1.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							// TODO Auto-generated method stub
							// apkDlMgr.ResDownloadStop(
							// CoolDLResType.RES_TYPE_APK ,
							// LongClickListenerDownloadItem.getPackageName() );
							...
							// // TODO: handle exception
							// MELOG.e( "ME_RTFSC" ,
							// "DownloadUpdateHander 4 Exception:" +
							// e.toString() );
							// }
				...
				t2.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						// TODO Auto-generated method stub
						LongClickListenerDialog.cancel();
						// apkDlMgr.ResDownloadStop( CoolDLResType.RES_TYPE_APK
						// , LongClickListenerDownloadItem.getPackageName() ,
						// true );
	...
	class InstallOnItemLongClickListener implements OnItemLongClickListener {
		...
		public boolean onItemLongClick(AdapterView<?> arg0, View arg1,
			...
			// TODO Auto-generated method stub
			MELOG.v("ME_RTFSC",
					"==== lvInstallListView.setOnItemLongClickListener===");
			LongClickListenerInstallItem = null;
			LongClickListenerDialog = null;
			LongClickListenerInstallItem = InstallApkList.get(arg2);
			LongClickListenerDialog = new AlertDialog.Builder(
					ApkMangerActivity.this).create();
			if (null != LongClickListenerInstallItem
					&& null != LongClickListenerDialog) {
				LayoutInflater inflaterDl = LayoutInflater
						.from(ApkMangerActivity.this);
				LinearLayout layout = (LinearLayout) inflaterDl.inflate(
						R.layout.cool_ml_onlongclick_listview_install, null);
				TextView t1 = (TextView) layout
						.findViewById(R.id.cool_ml_InstallListTextView1);
				TextView t2 = (TextView) layout
						.findViewById(R.id.cool_ml_InstallListTextView2);
				if (1 == LongClickListenerInstallItem.getInstallState()) {
					...
					t1.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							// TODO Auto-generated method stub
							dl_info info = MeapkDlMgr.GetSdkApkDlMgr()
									.ResGetInfo(
											CoolDLResType.RES_TYPE_APK,
											LongClickListenerInstallItem
													.getPackageName(), true);
							LongClickListenerDialog.cancel();
							if (info != null && info.IsDownloadSuccess()) {
								MeGeneralMethod.InstallApk(
										ApkMangerActivity.this,
										info.getFilePath());
							}
						}
					...
					t1.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							// MyMethod.StartActivityByPackageName(
							// getApplicationContext() ,
				...
				t2.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						// // TODO: handle exception
						// MELOG.e( "ME_RTFSC" ,
						// "InstallOnItemLongClickListener Exception:" +
						// e.toString() );
						// }
	...
	private void InitDownLoadListView() {
		...
		if (null != DownloadApkList && DownloadApkList.size() > 0) {
			downloadAdapter = new DownloadListViewAdapter(DownloadApkList,
					ApkMangerActivity.this);
			lvDownloadListView.setAdapter(downloadAdapter);
			lvDownloadListView
					.setOnItemLongClickListener(new DownloadOnItemLongClickListener());
		}
	}
	...
	private void InitInstallListView() {
		...
		if (null != InstallApkList && InstallApkList.size() > 0) {
			installAdapter = new InstallListViewAdapter(InstallApkList,
					ApkMangerActivity.this);
			lvInstallListView.setAdapter(installAdapter);
			lvInstallListView
					.setOnItemLongClickListener(new InstallOnItemLongClickListener());
		}
	}
	...
	protected void onDestroy() {
		...
		// MeCoolDLCallback.MeAddActiveCallBackMap.remove(
		// MeApkDLShowType.DownloadManagerActivity );
		MeApkDownloadManager.MeAddActiveCallBackMap
				.remove(MeApkDLShowType.DownloadManagerActivity);
	...
	private void initView() {
		...
		viewDownload = LayoutInflater.from(this).inflate(
				R.layout.cool_ml_apk_download_view, null);
		viewInstall = LayoutInflater.from(this).inflate(
				R.layout.cool_ml_apk_install_view, null);
		GridPagerAdapter adapter = new GridPagerAdapter(viewDownload,
				viewInstall);
	...
	class InstallControlButClicklister implements OnClickListener {
		...
		public InstallControlButClicklister(InstallApkItemEx CurItem) {
		...
		public void onClick(View v) {
			...
			if (1 == CurItem.getInstallState()) {
				dl_info info = MeapkDlMgr.GetSdkApkDlMgr().ResGetInfo(
						CoolDLResType.RES_TYPE_APK, CurItem.getPackageName(),
						true);
				if (info != null && info.IsDownloadSuccess()) {
					MeGeneralMethod.InstallApk(ApkMangerActivity.this,
							info.getFilePath());
				}
			} else if (2 == CurItem.getInstallState()) {
				StartActivityByPackageName(CurItem.getPackageName(),
						getApplicationContext());
			}
		}
	}
	...
	class InstallListViewAdapter extends BaseAdapter {
		...
		public InstallListViewAdapter(List<InstallApkItemEx> InstallApkList,
				Context context) {
			// TODO Auto-generated constructor stub
		...
		public void Update(List<InstallApkItemEx> InstallApkList) {
			this.InstallApkList = InstallApkList;
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = mInflater.inflate(
						R.layout.cool_ml_manager_install_listview, null);
				Holder.ivAppIcon = (ImageView) convertView
						.findViewById(R.id.cool_ml_manager_appIco1);
				Holder.tvAppName = (TextView) convertView
						.findViewById(R.id.cool_ml_manager_appName1);
				Holder.tvAppVersion = (TextView) convertView
						.findViewById(R.id.cool_ml_manager_appVersion1);
				Holder.tvAppSize = (TextView) convertView
						.findViewById(R.id.cool_ml_manager_appSize1);
				Holder.TvAppInstallControl = (TextView) convertView
						.findViewById(R.id.cool_ml_manager_button1);
				Holder.TvAppInstallControl
						.setOnClickListener(new InstallControlButClicklister(
								CurItem));
			...
			if (1 == CurItem.getInstallState()) {
				Holder.TvAppInstallControl
						.setBackgroundResource(R.drawable.cool_ml_icon_btn_list_install);
			} else {
				Holder.TvAppInstallControl
						.setBackgroundResource(R.drawable.cool_ml_icon_btn_list_run);
			}
			return convertView;
		}
	}
	...
	class DownloadListViewAdapter extends BaseAdapter {
		...
		public DownloadListViewAdapter(List<DownloadApkItem> DownloadApkList,
				Context context) {
			// TODO Auto-generated constructor stub
		...
		public void Update(List<DownloadApkItem> DownloadApkList) {
			this.DownloadApkList = DownloadApkList;
			notifyDataSetChanged();
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				// 根据自定义的Item布局加载布局
				convertView = mInflater.inflate(
						R.layout.cool_ml_manager_download_listview, null);
				Holder.ivAppIcon = (ImageView) convertView
						.findViewById(R.id.cool_ml_manager_appIco);
				Holder.tvAppName = (TextView) convertView
						.findViewById(R.id.cool_ml_manager_appName);
				Holder.progressBarApp = (ProgressBar) convertView
						.findViewById(R.id.cool_ml_manager_progressBar);
				Holder.tvAppSize = (TextView) convertView
						.findViewById(R.id.cool_ml_manager_appSize);
				Holder.TvAppDownloadControl = (TextView) convertView
						.findViewById(R.id.cool_ml_manager_button);
				Holder.TvAppDownloadControl
						.setOnClickListener(new DownloadControlButClicklister(
								CurItem));
				// 将设置好的布局保存到缓存中，并将其设置在Tag里，以便后面方便取出Tag
				convertView.setTag(Holder);
			} else {
			...
			Holder.tvAppName.setText(CurItem.getAppName());
			int press = (int) ((float) CurItem.getCurSize() * 100 / (float) CurItem
					.getTotalSize());
			Holder.progressBarApp.setProgress(press);
			Holder.tvAppSize.setText((int) (CurItem.getCurSize() >> 10) + "K/"
					+ (int) (CurItem.getTotalSize() >> 10) + "k");
			// DownLoadState 0:未在下载(none) 1:排队等待(wait) 2:正在下载(doing)
			// icon_btn_list_pause icon_btn_list_install icon_btn_list_download
			if (2 == CurItem.getDownLoadState()) {
				Holder.TvAppDownloadControl
						.setBackgroundResource(R.drawable.cool_ml_icon_btn_list_download);
			} else if (1 == CurItem.getDownLoadState()) {
				Holder.TvAppDownloadControl
						.setBackgroundResource(R.drawable.cool_ml_icon_btn_list_waiting_download);
			} else {
				Holder.TvAppDownloadControl
						.setBackgroundResource(R.drawable.cool_ml_icon_btn_list_pause);
			}
			return convertView;
		}
	}
	...
	class DownloadControlButClicklister implements OnClickListener {
		...
		public DownloadControlButClicklister(DownloadApkItem CurItem) {
			// TODO Auto-generated constructor stub
			// int DownLoadState; /* 0:初始化状态 1:下载中 2:正则下载 3:下载完成*/
			this.CurItem = CurItem;
		}
		...
		public void onClick(View v) {
			...
			// int DownLoadState; /* 0:初始化状态 1:下载中 2:正则下载 3:下载完成*/
			MELOG.v("ME_RTFSC",
					"====DownloadControlButClicklister  onClick  ==="
							+ CurItem.getDownLoadState()
							+ CurItem.getPackageName());
			// 首先刷新状态，再调用下载（停止）函数进行下载（停止）
			Message DownloadControlButClickMsg = new Message();
			// 4 -- 开始或者暂停下载
			...
			// {
			// // TODO: handle exception
			// MELOG.e( "ME_RTFSC" , "DownloadUpdateHander 4 Exception:" +
			// e.toString() );
			// }
		}
	}
	...
	class DownLoadCallBackMsgData {
		...
		public DownLoadCallBackMsgData(DownloadApkItem CurItem, dl_info DlInfo) {
			// TODO Auto-generated constructor stub
			this.CurItem = CurItem;
			this.DlInfo = DlInfo;
		}
	...
	class DownLoadCallBack extends MeApkDownloadCallBack {
		...
		public DownLoadCallBack(MeApkDLShowType showType,
				DownloadApkItem CurItem) {
			// TODO Auto-generated constructor stub
			super(showType);
			this.CurItem = CurItem;
		...
		@Override
		void onDoing(String PackageName, dl_info info) {
			// TODO Auto-generated method stub
			// MELOG.v( "ME_RTFSC" , "DownLoadCallBack:onDoing" );
			DownLoadCallBackMsgData msgData = new DownLoadCallBackMsgData(
					CurItem, info);
		...
		@Override
		void onSuccess(String PackageName, dl_info info) {
			// TODO Auto-generated method stub
			MELOG.v("ME_RTFSC",
					"APK  onSuccess  name:" + CurItem.getPackageName());
			Message DownloadOnDoingMsg = new Message();
		...
		@Override
		void onFail(String PackageName, dl_info info) {
			// TODO Auto-generated method stub
		...
		@Override
		void onRestart(String PackageName) {
			// TODO Auto-generated method stub
			...
			for (int i = 0; i < DownloadApkList.size(); i++) {
				if (DownloadApkList.get(i).getAppName().equals(PackageName)) {
					...
					MeapkDlMgr.AddCallback(
							MeApkDLShowType.DownloadManagerActivity,
							PackageName, curItem.getDownloadCallback());
					Message DownloadControlButClickMsg = new Message();
					DownloadControlButClickMsg.what = 4;
					DownloadControlButClickMsg.obj = curItem;
					DownloadUpdateHander
	...
	private class GridPagerAdapter extends PagerAdapter {
		...
		public GridPagerAdapter(View frist, View second) {
